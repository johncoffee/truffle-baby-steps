...
const pathToFile = (0, path_1.join)(__dirname, '../node_modules/truffle-typings/index.d.ts');
const wrong = `declare type Web3 = import("web3");`;
const right = `declare type Web3 = import("web3").default;`;
fix();
async function fix() {
    const file = await (0, fs_extra_1.readFile)(pathToFile);
    const content = file.toString();
    if (content.includes(wrong)) {
        console.log('Fixing file...');
        const fixed = content.replace(wrong, right);
        await (0, fs_extra_1.writeFile)(pathToFile, fixed);
        console.log('Fixed: ');
        console.log(right);
