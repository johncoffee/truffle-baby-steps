...
//    event Transition(bytes32 stateId, uint256 blockNumber);
    ...
    function setStates(bytes32[6] _stateIds) internal {
        ...
        require(_stateIds[0] != 0, "state must not be 0");
        ...
        for (uint256 i = 1; i < _stateIds.length; i++) {
            if (_stateIds[i] > 0) {
                ...
                require(states[_stateIds[i]].nextStateId == 0, "There must not be more than one of each state here");
            }
        }
    }
    ...
    function goToNextState() public {
        bytes32 next = states[currentStateId].nextStateId;
        require(next != 0, "Next state must not be 0");
